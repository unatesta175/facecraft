...
export default function PhotographerViewPage({ params }: { params: { id: string } }) {
    ...
    <AdminLayout>
      <div className="p-8 max-w-2xl space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1"><h1 className="text-xl font-semibold text-[--color-text-primary]">View Photographer</h1></div>
          <Button variant="outline" onClick={() => router.push(`/admin/photographers/${user.id}/photos`)}>
            <Image className="h-4 w-4 mr-2" />
            View Photos
          </Button>
          <Button onClick={() => router.push(`/admin/photographers/${user.id}/edit`)} className="bg-[--color-gold] hover:bg-[--color-gold]/90 text-white"><Pencil className="h-4 w-4 mr-2" />Edit</Button>
        </div>
        <div className="bg-white border border-[--color-border] rounded-xl p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Delete Permission" value={user.deletePermission ? 'Yes' : 'No'} />
          </div>
        </div>
      </div>
    </AdminLayout>
